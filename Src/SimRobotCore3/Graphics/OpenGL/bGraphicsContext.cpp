/**
 * @file Graphics/OpenGL/bGraphicsContext.cpp
 *
...
void bGraphicsContext::createGraphics()
{
...
void bGraphicsContext::destroyGraphics()
{
  const auto* context = QOpenGLContext::currentContext();
...
void bGraphicsContext::setClearColor(const float* color)
{
  std::memcpy(clearColor, color, sizeof(clearColor));
}

bGraphicsContext::IndexBuffer* bGraphicsContext::requestIndexBuffer()
{
  return indexBuffers.emplace_back(new IndexBuffer);
}

bGraphicsContext::Mesh* bGraphicsContext::requestMesh(const VertexBufferBase* vertexBuffer, const IndexBuffer* indexBuffer, PrimitiveTopology primitiveTopology)
{
  Mesh* mesh = new Mesh;
...
bGraphicsContext::Texture* bGraphicsContext::requestTexture(const std::string& file)
{
...
bGraphicsContext::Surface* bGraphicsContext::requestSurface(const float* diffuseColor, const float* ambientColor, const float* specularColor, const float* emissionColor, float shininess, const Texture* texture)
{
...
void bGraphicsContext::setGlobalAmbientLight(const float* color)
{
  globalAmbientLight = "vec4(" + std::to_string(color[0]) + ", " + std::to_string(color[1]) + ", " + std::to_string(color[2]) + ", " + std::to_string(color[3]) + ")";
}

void bGraphicsContext::addLight(const SimLight* light)
{
  //TODO
  /*ASSERT(lightDeclarations.size() == lightCalculations.size());
  if(const SimDirLight* dirLight = dynamic_cast<const SimDirLight*>(light); dirLight)
  {
  ...
  else if(const SimSpotLight* spotLight = dynamic_cast<const SimSpotLight*>(light); spotLight)
  {
  ...
  else if(const SimPointLight* pointLight = dynamic_cast<const SimPointLight*>(light); pointLight)
  {
...
void bGraphicsContext::updateModelMatrices(ModelMatrix::Usage usage, bool forceUpdate)
{
...
void bGraphicsContext::startColorRendering(const Matrix4f& projection, const Matrix4f& view, int viewportX, int viewportY, int viewportWidth, int viewportHeight, bool clear, bool lighting, bool textures, bool smoothShading, bool fillPolygons)
{
  const auto* context = QOpenGLContext::currentContext();
  ASSERT(!data);
  ASSERT(!shader);
...
void bGraphicsContext::startDepthOnlyRendering(const Matrix4f& projection, const Matrix4f& view, int viewportX, int viewportY, int viewportWidth, int viewportHeight, bool clear)
{
  const auto* context = QOpenGLContext::currentContext();
  ASSERT(!data);
  ASSERT(!shader);
...
void bGraphicsContext::setForcedSurface(const Surface* surface)
{
...
void bGraphicsContext::draw(const Mesh* mesh, const ModelMatrix* modelMatrix, const Surface* surface)
{
  ASSERT(data);
  ASSERT(shader);
  ASSERT(f);
...
void bGraphicsContext::finishRendering()
{
  ASSERT(data);
  ASSERT(shader);
  ASSERT(f);
...
bool bGraphicsContext::makeCurrent(int width, int height, bool sampleBuffers)
{
...
void bGraphicsContext::finishImageRendering(void* image, int w, int h)
{
...
void bGraphicsContext::finishDepthRendering(void* image, int w, int h)
{
  QOpenGLFunctions_3_3_Core* f = perContextData[QOpenGLContext::currentContext()].f;
...
void bGraphicsContext::setSurface(const Surface* surface)
{
  ASSERT(data);
  ASSERT(shader);
  ASSERT(f);
...
GLuint bGraphicsContext::compileShader(const std::vector<const char*>& vertexShaderSources, const std::vector<const char*>& fragmentShaderSources)
{
  ASSERT(f);
#ifndef NDEBUG
  GLint success = 0;
...
bGraphicsContext::Shader bGraphicsContext::compileColorShader(bool lighting, bool textures, bool smooth)
{
...
bGraphicsContext::Shader bGraphicsContext::compileDepthOnlyShader()
{
  const char* versionSourceCode = "#version 330 core\n";
...
QOpenGLFunctions_3_3_Core* bGraphicsContext::getOpenGLFunctions() const
{
...
void bGraphicsContext::VertexPN::setupVertexAttributes(QOpenGLFunctions_3_3_Core& functions)
{
...
void bGraphicsContext::VertexPNT::setupVertexAttributes(QOpenGLFunctions_3_3_Core& functions)
{
  functions.glEnableVertexAttribArray(0);
...
bGraphicsContext::Texture::Texture(const std::string& file)
{
  QImage image;
...
bGraphicsContext::Texture::~Texture()
{
  delete[] data;
}

void bGraphicsContext::ModelMatrix::updateMemory()
{
  if(!variablePart)
    return;

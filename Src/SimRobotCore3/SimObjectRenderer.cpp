...
#include <iostream>
//#include <Desktop/bGraphicsContext.h>
...
void SimObjectRenderer::draw()
{
  ...
  if(drawAppearances || drawControllerDrawings)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::appearance, dragging && dragSelection);
  if(drawPhysics || drawControllerDrawings)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::physicalDrawing, dragging && dragSelection);
  if(drawSensors || drawControllerDrawings)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::sensorDrawing, dragging && dragSelection);
  if(drawControllerDrawings)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::controllerDrawing, dragging && dragSelection);
  if(drawDragPlane)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::dragPlane, true);
  ...
  if(drawCoordinateSystem)
    graphicsContext.updateModelMatrices(bGraphicsContext::ModelMatrix::origin, true);
    ...
    if(dragSelection)
    {
      ...
      else
      {
        ...
        if(dragMode == resetDynamics)
          static_cast<SimRobotCore3::Body*>(dragSelection)->resetDynamics();
...
bool SimObjectRenderer::releaseDrag(int x, int y)
{
  ...
  else // object control
  {
    ...
    static_cast<SimRobotCore3::Body*>(dragSelection)->enablePhysics(true);
